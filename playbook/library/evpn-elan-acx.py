...
#    print("set groups {} interfaces {} esi {}".format(group,interface,esi))
    ...
    for i in range(int(vlan_start), int(vlan_end)+1):
        print("set groups {} routing-instances evpn_elan_{:d} instance-type mac-vrf".format(group,i))
        print("set groups {} routing-instances evpn_elan_{:d} protocols evpn encapsulation mpls".format(group,i))
        print("set groups {} routing-instances evpn_elan_{:d} service-type vlan-based".format(group,i))
        print("set groups {} routing-instances evpn_elan_{:d} protocols evpn no-control-word".format(group,i))
        ...
        print("set groups {} routing-instances evpn_elan_{:d} vlans bd{:d} vlan-id {:d}".format(group,i,i,i))
        print("set groups {} routing-instances evpn_elan_{:d} vlans bd{:d} interface {}.{:d}".format(group,i,i,interface,i))
