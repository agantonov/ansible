...
def generate_config():
    ...
    if len(sys.argv) != 10:
        print("Usage: {} router_id interface esi_base address_base_v4 address_base_v6 step site-id vlan_id_start vlan_id_end".format(sys.argv[0]))
        print("Example: {} 1.1.0.7 ae17 00:17:00:00:00:00:00:00:01:00 19.0.0.1 2001:19::1 1 10 100 200".format(sys.argv[0]))
        exit(1)
    ...
    for i in range(int(vlan_start), int(vlan_end) + 1):
        ...
        print("set groups {} routing-instances evpn_irb_{:d} instance-type mac-vrf".format(group,i))
        print("set groups {} routing-instances evpn_irb_{:d} protocols evpn default-gateway no-gateway-community".format(group,i))
        print("set groups {} routing-instances evpn_irb_{:d} service-type vlan-based".format(group,i))
        ...
        print("set groups {} routing-instances evpn_irb_{:d} vlans bd{:d} vlan-id {:d}".format(group,i,i,i))
        print("set groups {} routing-instances evpn_irb_{:d} vlans bd{:d} interface {}.{:d}".format(group,i,i,interface,i))
        print("set groups {} routing-instances evpn_irb_{:d} vlans bd{:d} l3-interface irb.{:d}".format(group,i,i,i))
...
#        esi = esi + 1
        ...
        #address_v6 = address_v6 + int("ffffffffffffffffffff", 16)
